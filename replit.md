...
- **User Presence**: Real-time online/offline status tracking via Socket.IO. Server maintains in-memory presence store with connect/disconnect/ping events. Stale session cleanup runs every 30 seconds (60-second threshold). REST endpoint at `/api/v1/presence` provides initial data; BULK_UPDATE sent on socket connection for reconnect rehydration. PresenceIndicator component displays green dot (online) or hollow circle (offline). Integrated across chat UI: team list, channel members, DM threads, mention popup, and context panels.
