...
- **User Presence**: Real-time online/idle/offline status tracking via Socket.IO. Server maintains in-memory presence store with connect/disconnect/ping/idle events. Status types: "online" (active with recent activity), "idle" (connected but inactive for 5+ minutes), "offline" (no active sockets). Client-side idle detection tracks mouse/keyboard/touch/scroll activity and page visibility - emits `presence:idle` event after 5 minutes of inactivity or when tab is hidden. Stale session cleanup runs every 30 seconds (60-second threshold). REST endpoint at `/api/v1/presence` provides initial data with status field; BULK_UPDATE sent on socket connection for reconnect rehydration. PresenceIndicator component displays: green dot (online), amber dot (idle), hollow gray circle (offline). Integrated across chat UI: team list, channel members, DM threads, mention popup, and context panels.
...
- **Project Templates**: Admin-only template editor for creating reusable project structures. Templates store sections with tasks as JSONB (`{ sections: [{ name, tasks: [{ title, description?, subtasks? }] }] }`). Features visual editor for sections/tasks and bulk paste-to-create using markdown format (`## Section` headers, `- Task` items). API at `/api/v1/templates` with full CRUD operations, tenant-scoped.
